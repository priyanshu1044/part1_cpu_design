...
int main(int argc, char* argv[]) {
    ...
    // In this pass, we scan the source file to identify all labels and assign them
    // addresses. This allows us to resolve forward jumps in Pass 2.
    while (getline(infile, line)) {
        ...
        if (opcode == "NOP" || opcode == "HALT") currentAddress += 1;
        else if (opcode == "MOVI") currentAddress += 4;
        else if (opcode == "JMP" || opcode == "JZ" || opcode == "JNZ") currentAddress += 3;
        else if (opcode == "OUT") currentAddress += 4;
        else if (opcode == "LOAD" || opcode == "STORE") currentAddress += 4; // Op + Reg + Addr
        else currentAddress += 3; // Op + R1 + R2
    }

    ofstream outfile(argv[2], ios::binary);
    
    // Pass 2: Generate Code
    // In this pass, we translate the instructions into machine code.
    // We use the symbol table (labels map) created in Pass 1 to resolve addresses.
    for (const auto& instr : instructions) {
        ...
        if (instr.opcode == "NOP" || instr.opcode == "HALT") continue;
        ...
        } else if (instr.opcode == "JMP" || instr.opcode == "JZ" || instr.opcode == "JNZ") {
